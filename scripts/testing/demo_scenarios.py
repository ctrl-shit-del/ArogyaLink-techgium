...
Calibration demo:
  ...
  Both reach HR=95  →  athlete: >2.5σ (PRE-ALERT)  |  hypertensive: <0.5σ (normal)
...
# ─────────────────────────────────────────────────────────────────────────────
# Model loader (falls back to rule-based if no checkpoint)
# ─────────────────────────────────────────────────────────────────────────────

def _load_model_or_none():
    """Try to load trained autoencoder; return None if not available."""
    ...
    try:
        ...
        model = VitalAutoencoder.load(str(ckpt))
...
class SyneraSimulationDemo:
    ...
    Each scenario has THREE distinct phases:
      Phase 1 — Silent calibration  : personalised baseline learned (50 readings)
      Phase 2 — Stable monitoring   : confirm near-zero false-positive rate
      Phase 3 — Event monitoring    : scenario-specific event fires
    """

    PRE_ALERT_SIGMA  = 2.5
    STATIC_HR_THRESH = 120
    CALIB_READINGS   = 50
...
def main():
    ...
    parser.add_argument("--calibration-demo", action="store_true")
    parser.add_argument(
        "--scenario",
